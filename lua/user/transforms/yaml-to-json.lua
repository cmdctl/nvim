...
M.execute = function()
  ...
  local jobid = vim.fn.jobstart("conv yaml json", {
    stdout_buffered = false,
    on_stdout = append_data,
    on_stderr = append_data,
    on_exit = function(_, code, _)
      if code == 0 then
        vim.api.nvim_buf_set_option(curr_buf, 'filetype', 'json')
      end
    end,
  })
...
vim.api.nvim_create_user_command('YamlToJson', M.execute, { nargs = 0 })
